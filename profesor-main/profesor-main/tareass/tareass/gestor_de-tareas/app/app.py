...
def init_db():
    ...
    # NEW: Create a sample student if they don't exist, assign them to a course
    cur.execute('SELECT COUNT(*) FROM usuarios WHERE nombre_usuario="estudiante1"')
    if not cur.fetchone()[0]:
        cur.execute('''
        INSERT INTO usuarios (nombre, nombre_usuario, curso, documento, correo, contrasena, rol)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            'Maria Lopez', 'estudiante1', '6A', '98765432',
            'estudiante1@example.com', generate_password_hash('estudiante123'),
            'rol_usuario'
        ))
        print("Usuario 'estudiante1' (Maria Lopez, Curso 6A) creado.")

    cur.execute('SELECT COUNT(*) FROM usuarios WHERE nombre_usuario="estudiante2"')
    if not cur.fetchone()[0]:
        cur.execute('''
        INSERT INTO usuarios (nombre, nombre_usuario, curso, documento, correo, contrasena, rol)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            'Pedro Gomez', 'estudiante2', '6A', '87654321',
            'estudiante2@example.com', generate_password_hash('estudiante123'),
            'rol_usuario'
        ))
        print("Usuario 'estudiante2' (Pedro Gomez, Curso 6A) creado.")

    cur.execute('SELECT COUNT(*) FROM usuarios WHERE nombre_usuario="estudiante3"')
    if not cur.fetchone()[0]:
        cur.execute('''
        INSERT INTO usuarios (nombre, nombre_usuario, curso, documento, correo, contrasena, rol)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            'Ana Torres', 'estudiante3', '7B', '76543210',
            'ana.torres@example.com', generate_password_hash('estudiante123'),
            'rol_usuario'
        ))
...
def login():
    if request.method == 'POST':
        ...
        if row and check_password_hash(row['contrasena'], p) and row['activo']:
            ...
            destino = rol_destinos.get(user.rol)
            if destino:
...
def obtener_estudiantes_por_curso(curso):
    # Ensure only professors can access this data
    if current_user.rol != 'rol_profesor':
        return jsonify({'error': 'Acceso denegado. Solo profesores pueden ver estudiantes.'}), 403
    
    conn = get_db_connection()
    try:
        # Fetch users with the specified course and the 'rol_usuario' role
        estudiantes = conn.execute(
            "SELECT id, nombre, nombre_usuario, documento, correo FROM usuarios WHERE curso = ? AND rol = 'rol_usuario' ORDER BY nombre",
            (curso,)
        ).fetchall()
        
        # Convert Row objects to dictionaries for JSON serialization
        estudiantes_dict = [dict(row) for row in estudiantes]
        return jsonify(estudiantes_dict)
    except Exception as e:
        print(f"Error fetching students for course {curso}: {e}")
        return jsonify({'error': f'Error al obtener estudiantes: {e}'}), 500
    finally:
        conn.close()
...
def rol_usuario():
    ...
    'SELECT * FROM tareas WHERE LOWER(curso_destino) = LOWER(?) ORDER BY fecha_vencimiento ASC',
    (usuario_curso,)
).fetchall()

    # ---- DEBUG PRINTS ----
    print("-"*50)
    print(f"Usuario logueado: {current_user.nombre_usuario}")
    print(f"Curso del usuario: {usuario_curso}")
    print(f"Tareas encontradas para este curso:")
...
def ver_tarea(tarea_id):
    # Solo usuarios normales pueden ver el detalle de tareas de su curso
    if current_user.rol != 'rol_usuario':
        flash('Acceso denegado.', 'danger')
        return redirect(url_for('index'))
    
    conn = get_db_connection()
    # Buscar la tarea sólo si pertenece al curso del usuario actual
    tarea = conn.execute(
        'SELECT * FROM tareas WHERE id = ? AND curso_destino = ?',
        (tarea_id, current_user.curso)
    ).fetchone()
    conn.close()
    if not tarea:
        flash('Tarea no encontrada o no tienes permiso para verla.', 'danger')
...
@app.route('/api/tarea_estudiante/<int:task_id>', methods=['GET'])
@login_required
def api_get_tarea_estudiante(task_id):
    if current_user.rol != 'rol_usuario':
        return jsonify(error="Acceso denegado."), 403
...
if __name__=='__main__':
    ...
    init_db() # Ensure database and default users are set up
    app.run(debug=True, port=5000) # This line was missing or incomplete